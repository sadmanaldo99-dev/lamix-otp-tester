...
# Environment Variables থেকে ডাটা রিড
TELEGRAM_BOT_TOKEN = (os.environ.get("BOT_TOKEN") or os.environ.get("TELEGRAM_BOT_TOKEN") or "").strip()
PUBLIC_CHANNEL_ID = (os.environ.get("CHAT_ID") or "").strip()
...
# ১. ইনপুট ফরম্যাট পার্সিং (ebay:972)
...
async def run_ai_browser_agent(service, country_range, phone_number, email):
    if not GROQ_API_KEY:
        log("Groq API Key is missing! AI Browser Agent skipped.")
        return False
    ...
    try:
        ...
        prompt = (
            f"Go to the sign up or registration page of {service}. "
            f"Enter email {email} if needed. "
            f"Find the mobile number input field, choose country code/range {country_range}, "
    ...
    for service, country_range in tasks:
        ...
        for attempt in range(1, 4):
            ...
            while True:
                ...
                await context.bot.send_message(
                    chat_id=chat_id, text="🔄 Re-checking stock..."
                )
            ...
            await context.bot.send_message(
                ...
                text=f"🧪 *Attempt {attempt}/3*\n"
                     ...
                     f"🤖 *AI Agent navigating to `{service}` site to submit number...*",
                parse_mode="Markdown",
            )

            # ব্যাকগ্রাউন্ডে ব্রাউজার এজেন্ট রান
...
async def handle_messages(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text.lower() == "added":
        if chat_id in waiting_events:
            ...
            await update.message.reply_text("👍 Resuming search...")
        return
    ...
    if not tasks:
        await update.message.reply_text(
            "⚠️ Invalid format! Send like this:\n`ebay:972`\n`amazon:1`",
            parse_mode="Markdown"
        )
        return
...
def main():
    if not TELEGRAM_BOT_TOKEN:
        log("CRITICAL ERROR: BOT_TOKEN is missing or empty!")
        return
    ...
    log("AI Automated Bot Started Successfully...")
    app.run_polling()
